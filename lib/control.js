...
steal("can/control", function () {
    ...
    can.Control.prototype.renderJig = function(configSelector, selector, options) {
        ...
        if (!Yd.config || !Yd.config.jigs || !Yd.config.jigs[configSelector]) {
            throw new Error("No config for " + configSelector);
            return;
        }

        options = can.extend(true, {}, Yd.config.jigs[configSelector].options, options);

        Controller = eval(Yd.config.jigs[configSelector].controller);
        if (Controller) {
    ...
    can.Control.prototype.helper = {
        ...
        browserNotification: function (title, body, onclick) {
        ...
        browserNotificationPermission: function () {
            if ("Notification" in window) {
