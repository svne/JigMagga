...
![JigMagga](media/img/JigMaggaWorkflow2.jpg)
...
Details about HTML-worker could be found [here][1]


Js/Css-Builder
------------ 

You can build your css and js production files with the build tool. You have to execute following in JigMagga folder.

    node build -p ".*" -n jm -d domain.com -v "0.1" -u -c true -j true -B bucket.name.com
...
In order to upload media files execute previous command with `-M` flag.

    node build -p ".*" -n jm -d domain.com -v "0.1" -u -M -B bucket.name.com
